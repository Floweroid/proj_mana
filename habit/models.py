...
class Habit(models.Model):
    ...
    def streak(self, user_timezone=None):
        ...
        print(f"Today's date in user's timezone ({user_timezone}): {today}")  # Debugging line
        ...
        for day in days:
            ...
            print(f"Comparing track date: {track_date} with today: {today}")  # Debugging line

            if track_date == today:
                streak_count += 1
                today -= timedelta(days=1)
                print(f"Streak incremented to {streak_count}. Next date to check: {today}")  # Debugging line
            else:
                print("Break in streak detected.")  # Debugging line
                break
