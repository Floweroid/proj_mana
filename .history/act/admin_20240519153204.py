...
class SubRequirementInline(admin.StackedInline):
    ...
    fk_name = 'parent'
    extra = 1
    fields = ('name', 'status', 'priority', 'parents', 'start_time', 'end_time','created_at', 'updated_at')
...
@admin.register(Requirement)
class RequirementAdmin(admin.ModelAdmin):
    list_display = ('name', 'status', 'priority', 'parents', 'start_time', 'end_time','created_at', 'updated_at')  # Display these fields in the list view
    readonly_fields = ('created_at', 'updated_at')
