...
class RequirementAdmin(admin.ModelAdmin):
    ...
    formats = ['%H:%M:%S %Y-%m-%d',  # Year-Month-Day Hour:Minute:Second
               '%Y-%m-%d',            # Year-Month-Day
               '%m/%d/%Y %I:%M %p']   # Month/Day/Year Hour:Minute AM/PM

    def changelist_view(self, request, extra_context=None):
        # Apply custom formats to the admin
        formats = self.formats + list(admin.ModelAdmin.formats)
        admin.ModelAdmin.formats = list(set(formats))
